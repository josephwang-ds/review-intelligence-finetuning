...
- **Practical effect on the numbers:** both benchmark tables in the README are now re-measured on `deepseek-v4-flash`. ASAP moved modestly lower (F1 0.757→0.696) with roughly 4x the latency — a real, disclosed result of the model change, not a regression in this codebase. Yelp actually moved *up* (F1 0.642→0.735) with a similar latency increase. Net effect: the "few-shot hurts English sentiment" finding in the original cross-lingual comparison didn't reproduce on the new model and has been retracted from the README rather than left in — it was a property of the retired model's specific behavior, not of the task.
